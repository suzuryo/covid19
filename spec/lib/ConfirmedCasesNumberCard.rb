...
def has_confirmed_cases_number_card
  ...
  # 陽性者数
  d = number_to_delimited(POSITIVE_RATE_JSON['data'].last['positive_count'])
  ...
  # テーブルの最上部の値(陽性者・累計)
  d = number_to_delimited(POSITIVE_RATE_JSON['data'].reduce(0) { |sum, n| sum + n['positive_count'].to_i })
